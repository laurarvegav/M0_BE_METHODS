...
def check_stock (quantity , name_of_item)
    if quantity >= 4
        puts stocked_status = "#{name_of_item} is stocked"
    elsif ((quantity < 4) && (quantity >= 1))
        puts stocked_status = "#{name_of_item} - running LOW"
    else 
        puts stocked_status = "#{name_of_item} - OUT of stock!"
    end 
end 
